# --------------------------------------------------------------------------
# Description: Cog that houses server moderator-only commands for JARVIS Bot
# --------------------------------------------------------------------------
...
class ActionReason(commands.Converter): # Look into this further, could be pretty useful for other things.
...
class ModCog(commands.Cog):
    ...
    async def purge(self, ctx, purge_amount: int):
        ...
        if purge_amount == 0:
            embed = discord.Embed(colour=self.bot.config.BOT_ERR_COLOUR) # Not sure why I made the error messages like this, ERROR should be in the title
            ...
            await ctx.message.delete() # Fix this, it isn't getting appended in audit logs (I don't think I'm allowed to append reasons to messages actually)
            await asyncio.sleep(5)
            await zeroerror.delete() # Look into delete_after argument, could replace asyncio.sleep statements in this cog
            return

        elif purge_amount < 0:
            embed = discord.Embed(colour=self.bot.config.BOT_ERR_COLOUR) # Change colour to red
            embed.set_author(name='J.A.R.V.I.S. Bot', url=self.bot.config.BOT_URL, icon_url=self.bot.user.avatar_url)
            embed.add_field(name='ERROR', value=f'You cannot delete a negative number of messages, nice try.', inline=False)
            embed.set_footer(text=self.bot.config.BOT_FOOTER)
            ...
            embed = discord.Embed(colour=self.bot.config.BOT_ERR_COLOUR)
            embed.set_author(name='J.A.R.V.I.S. Bot', url=self.bot.config.BOT_URL, icon_url=self.bot.user.avatar_url)
            embed.add_field(name='ERROR', value=f'For security reasons you cannot delete more than 100 messages at a time, please try again.', inline=False)
            ...
            embed.set_author(name='J.A.R.V.I.S. Bot', url=self.bot.config.BOT_URL, icon_url=self.bot.user.avatar_url)
            embed.add_field(name='Purge Confirmation', value=f'Please confirm (yes/no) that you want to clear ``{purge_amount}`` message(s)', inline=False)
            embed.set_footer(text = f'Actioned by {ctx.author.name}#{ctx.author.discriminator}', icon_url=(ctx.author.avatar_url))
        ...
        while True:
            try:
                ...
                if msg.content.lower() == "yes" and msg.author == ctx.author:
                    embed = discord.Embed(title='WARNING', colour=self.bot.config.BOT_ERR_COLOUR)
                    embed.set_author(name='J.A.R.V.I.S. Bot', url=self.bot.config.BOT_URL, icon_url=self.bot.user.avatar_url)
                    embed.add_field(name='Purge In Progress', value=f'Clearing ``{purge_amount}`` message(s)...', inline=False)
                    embed.set_footer(text = f'Actioned by {ctx.author.name}#{ctx.author.discriminator}', icon_url=(ctx.author.avatar_url))
                    ...
                    await msg.delete()
                    await ctx.message.delete()
                    deleted = await ctx.channel.purge(limit=purge_amount, before=ctx.message) # Experiment with bulk=True/False, maybe let the user input if they want bulk delete or not to preserve messages in logs (bulk delete is faster)
